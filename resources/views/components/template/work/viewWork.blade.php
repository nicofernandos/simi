    ...
    <div class="card mt-4 shadow-md">
        <div class="flex px-2 py-4 border-b-2 border-indigo-500">
            <div class="flex items-center bg-slate-200 mx-2 my-2 gap-2">
                ...
                <span class="text-gray-700 font-semibold">{{ $works->id }}</span>
            </div>
            <div class="flex items-center bg-slate-200 mx-2 my-2 gap-2">
                <span class="text-gray-800 text-warp">Nomor Pekerjaan</span>
                <span class="text-gray-700 font-semibold hover:text-blue-600 hover:underline">{{ $works->no_task }}</span>
            </div>
        </div>
        ...
        <div class="flex grid grid-cols-4 px-2 py-4 gap-4">
            ...
            <span class="bg-slate-600 card max-w-full px-2 py-3 rounded-2xl shadow-md">
                <div class="bg-slate-200 font-semibold">: {{ $works->user->name }}</div>
            </span>
            <div class="bg-slate-600 px-2 py-3">Departemen </div>
            <span class="bg-slate-600 max-w-full card px-2 py-3 rounded-2xl shadow-md">
                <div class="bg-slate-200 font-semibold ">: {{ $works->user->department->name }}</div>
            </span>
            <div class="bg-slate-600 px-2 py-3">Pekerjaan</div>
            <span class="bg-slate-600 card max-w-full px-2 py-3 rounded-2xl shadow-md">
                <div class="bg-slate-200  font-semibold">: {{ $works->title }}</div>
            </span>
            <div class="bg-slate-600 px-2 py-3">Deskripsi pekerjaan</div>
            <span class="bg-slate-600 card max-w-full px-2 py-3 rounded-2xl shadow-md">
                <div class="bg-slate-200 font-semibold">: {{ $works->description }}</div>
            </span>
            <div class="bg-slate-200 px-2 py-3">Status </div>
            <span class="bg-slate-600 card max-w-full px-2 py-3 rounded-2xl shadow-md 
                @if($works->status == 'pending') bg-green-600 text-white shadow-sm
                @elseif($works->status == 'selesai') bg-red-900 text-white shadow-sm
                @else bg-yellow-600 text-white @endif">
                <div class="bg-slate-200 font-semibold rounded-sm ">: {{ ucfirst($works->task_status) }}</div>
            </span>
            <div class="bg-slate-200 px-2 py-3">Total blok </div>
            <span class="bg-slate-600 card max-w-full px-2 py-3 rounded-2xl shadow-md">
                <div class="bg-slate-200 font-semibold">: {{ $works->blok }} Blok</div>
            </span>
            <div class="bg-slate-200 px-2 py-3">Tanggal Pekerjaan </div>
            <span class="bg-slate-600 card max-w-full px-2 py-3 rounded-2xl shadow-md">
                <div class="bg-slate-200 font-semibold">: {{ $works->created_at }}</div>
            </span>
            <div class="bg-slate-200 px-2 py-3">Tanggal Pekerjaan Selesai </div>
            <span class="bg-slate-600 card max-w-full px-2 py-3 rounded-2xl shadow-md">
                <div class="bg-slate-200 font-semibold">: {{ $works->date }}</div>
            </span>
        ...
        <div class="grid grid-cols-3 gap-4 px-4 py-3">
            ...
            <div class="text-blue-600 text-center">
                <div>Pertama</div>
                <img class="w-32 h-32 mt-2 mx-auto shadow-xl rounded-lg" src="{{ $works->dokumentasi ? asset('storage/' . $works->dokumentasi) : asset('img/default.jpg') }}" alt="Pertama">
            </div>
        
            <!-- Dokumentasi 2 -->
            <div class="text-blue-600 text-center">
                <div>Kedua</div>
                <img class="w-32 h-32 mt-2 mx-auto shadow-xl rounded-lg" src="{{ $works->dokumentasi1 ? asset('storage/' . $works->dokumentasi1) : asset('img/default.jpg') }}" alt="Kedua">
            </div>
        
            <!-- Dokumentasi 3 -->
            <div class="text-blue-600 text-center">
                <div>Ketiga</div>
                <img class="w-32 h-32 mt-2 mx-auto shadow-xl rounded-lg" src="{{ $works->dokumentasi2 ? asset('storage/' . $works->dokumentasi2) : asset('img/default.jpg') }}" alt="Ketiga">
            </div>
        </div>
        ...
        <div class="flex mt-2 grid grid-flow-col-2 px-2 py-4 gap-4">
            ...
            <div class="bg-slate-200 font-semibold"> {{ $works->uraian_pekerjaan }}</div>
        </div>
